...
class PokerGame:
        ...
        try:
            ...
            for i in range(12):
                if self.pokerkit_state.can_deal_hole() and card_index < len(deck):
                    ...
                    print(f"Cannot deal card {i+1}, stopping")
                    break
                    
        except Exception as e:
            print(f"Error dealing cards: {e}")
            # Fallback: create mock hole cards if PokerKit fails
            self._create_fallback_cards()
    
    def _create_fallback_cards(self):
        """Create fallback hole cards if PokerKit dealing fails"""
        try:
            # Create unique cards for each player as fallback
            ranks = ['2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K', 'A']
            suits = ['h', 'd', 'c', 's']
            deck = [f"{rank}{suit}" for rank in ranks for suit in suits]
            random.shuffle(deck)
            
            # Manually assign unique cards to each player
            if not hasattr(self.pokerkit_state, 'hole_cards') or not self.pokerkit_state.hole_cards:
                # Create mock hole cards structure
                self.fallback_hole_cards = []
                for player_idx in range(6):
                    player_cards = [deck[player_idx * 2], deck[player_idx * 2 + 1]]
                    self.fallback_hole_cards.append(player_cards)
                print("Created fallback hole cards")
        except Exception as e:
    ...
    def _initialize_unique_cards(self):
        ...
        try:
            ...
            # Ultimate fallback - create simple unique cards
            self.unique_hole_cards = [
                ['Ah', 'Kh'], ['Qh', 'Jh'], ['Th', '9h'],
                ['8h', '7h'], ['6h', '5h'], ['4h', '3h']
            ]
    ...
    def _create_api_state(self) -> GameState:
        ...
        for i in range(6):
            ...
            # Get hole cards for this player using unique card system
            hole_cards = []
            
            # First priority: Use unique card system
            if hasattr(self, 'unique_hole_cards') and self.unique_hole_cards and i < len(self.unique_hole_cards):
                try:
                    unique_cards = self.unique_hole_cards[i]
                    hole_cards = [self._pokerkit_card_to_api_card(card) for card in unique_cards]
                except Exception as e:
                    print(f"Error using unique cards for player {player_id}: {e}")
            
            # Second priority: Try PokerKit cards (if unique cards failed)
            if not hole_cards and (self.pokerkit_state and 
                ...
                try:
                    ...
                    print(f"Error converting PokerKit hole cards for player {player_id}: {e}")
            
            # Third priority: Use fallback cards
            if not hole_cards and hasattr(self, 'fallback_hole_cards') and self.fallback_hole_cards:
                try:
                    if i < len(self.fallback_hole_cards):
                        fallback_cards = self.fallback_hole_cards[i]
                        hole_cards = [self._pokerkit_card_to_api_card(card) for card in fallback_cards]
                except Exception as e:
                    print(f"Error using fallback cards for player {player_id}: {e}")
